...
const PortfolioDropdown = ({ portfolios, activePortfolio, setActivePortfolio, navigate, location }) => {
  ...
  const standardPortfolios = filteredPortfolios.filter(p => p.type === 'standard');
  const aiPortfolios = filteredPortfolios.filter(p => p.type === 'ai_builtin' || p.type === 'ai_custom');
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="dropdown-modern modern-scrollbar min-w-[240px]"
          >
            <div className="p-2 border-b border-white/5">
              <div className="relative">
                <input
                  ...
                  className="w-full bg-slate-900/50 border border-white/10 rounded-lg py-1.5 pl-8 pr-3 text-xs focus:outline-none focus:ring-1 focus:ring-emerald-500/50 placeholder:text-slate-600"
                  autoFocus
                />
      ...
      >
        ...
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="hidden sm:flex flex-col justify-center">
            <h1 className="text-xl font-black leading-tight bg-clip-text text-transparent bg-gradient-to-r from-white to-slate-300">
              ChatSense
            </h1>
            {/* <motion.span
              initial={{ opacity: 0 }}
            ...
            <AnimatePresence>
              {showUserMenu && (
                <motion.div
                  ...
                  animate={{ opacity: 1, scale: 1, y: 0 }}
                  exit={{ opacity: 0, scale: 0.95, y: 10 }}
                  className="dropdown-modern right-0 w-48 mt-3 py-2"
                >
