...
export default function SentimentPortfolioSelect() {
    ...
    useEffect(() => {
        ...
        const fetchAllPortfolios = async () => {
            try {
                ...
                // Mark sectors clearly
                const sectors = sectorRes.data.map(s => ({ ...s, type: 'sector' }));
                const combined = [...portRes.data, ...sectors];
    ...
    const standardPortfolios = portfolios.filter(p => p.type === 'standard' || (!p.type && !p.id.includes('builtin')));
    ...
    const PortfolioCard = ({ portfolio, index }) => (
        ...
        >
            ...
            <div className="relative z-10">
                <div className={`w-12 h-12 ${portfolio.type === 'sector' ? 'bg-amber-500/10' : 'bg-blue-500/10'} rounded-xl flex items-center justify-center mb-4 text-2xl group-hover:scale-110 transition-transform`}>
                    {portfolio.type === "ai_builtin" || portfolio.type === "ai_custom" ? "✨" : portfolio.type === 'sector' ? "📊" : "📁"}
                </div>
                ...
                <div className="mt-6 flex items-center justify-between border-t border-white/5 pt-4">
                    <div className="flex flex-col">
                        <span className={`text-[10px] font-bold uppercase tracking-widest ${portfolio.type === 'sector' ? 'text-amber-400' : 'text-slate-500'}`}>
    ...
    return (
        <div className="space-y-12">
            ...
            <div className="space-y-16">
                ...
                {sectorPortfolios.length > 0 && (
                    <section>
                        <div className="flex items-center gap-4 mb-6">
                            <h3 className="text-sm font-bold text-amber-400 uppercase tracking-[0.2em] flex items-center gap-2">
                                <span className="w-6 h-0.5 bg-amber-500 rounded-full"></span>
                                Sector Portfolios
                            </h3>
                            <span className="text-slate-600 text-[10px] font-black">{sectorPortfolios.length} AVAILABLE</span>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {sectorPortfolios.map((p, i) => (
                ...
                {/* AI & Custom Portfolios Section */}
                {(aiPortfolios.length > 0 || standardPortfolios.length > 0) && (
                    <section>
                        <div className="flex items-center gap-4 mb-6">
                            <h3 className="text-sm font-bold text-blue-400 uppercase tracking-[0.2em] flex items-center gap-2">
                                <span className="w-6 h-0.5 bg-blue-500 rounded-full"></span>
                                Custom & AI Portfolios
                            </h3>
                            <span className="text-slate-600 text-[10px] font-black">{aiPortfolios.length + standardPortfolios.length} WORKSPACES</span>
                        </div>
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            {[...aiPortfolios, ...standardPortfolios].map((p, i) => (
                                <PortfolioCard key={p.id} portfolio={p} index={i} />
                            ))}
                        </div>
                    </section>
